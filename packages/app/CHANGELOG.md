...
- 497d50b40: feat: Allow selecting the column or SQL expression used for event pattern grouping (with shareable URL state)
...
- 052315b1a: fix: improve contrast of excluded search filter pills
...
- 8261b4618: fix: inline parametric aggregate function arguments instead of passing as query parameters
...
- b763ba646: fix: next-runtime-env runtime env var injection fixed for images
...
- 48e19e8be: Suggest existing section names in the source form's **Section** field. The field is now an autocomplete fed by the sections already in use, so a new source can reuse an existing section instead of retyping it (which is how a section ends up split into near-duplicates like "Billing" and "billing"). The field stays free-text, so any new section name is still accepted.
...
- Updated dependencies [8261b4618]
